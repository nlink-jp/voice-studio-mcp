...
`script_path` や `casting_path` が `.env`、このサーバーの設定ディレクトリ内、資格情報
ディレクトリ内のリンクの行き先のいずれかなら、読む前に `path_not_allowed` で拒否します。
ファイルがあってもなくても同じ答えです。ワークスペースはそうした場所を含み得て
(`~/.ssh/config` がリンクする同期フォルダ内など)、台本として読むと中身が解析エラーに
出てしまうためです。
