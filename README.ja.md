...
> AIエージェント向けに、多話者**日本語**音声合成をローカルで行う MCP
> サーバー(単一バイナリ、AivisSpeech Engine バックエンド)。多話者の台本を
> 完成音声に変換します(ラジオドラマ・オーディオブック・ポッドキャスト・
> パネル・ブリーフィング 等)。
>
> **対応言語は日本語のみ。** AivisSpeech Engine は日本語を合成するため、
> 他言語には対応していません。

## なぜ作ったか

台本を多話者の音声にする作業は2種類に分かれます。*知的作業*(脚本化・
...
ラジオドラマとオーディオブックが最初のユースケースで、同梱の
multi-actor-narration スキルがポッドキャスト・パネル・ニュース・教材を
加えます。クラウドAPI・クレデンシャル不要です。

エンジンの音声モデルは**日本語**のため、voice-studio-mcp が生成できるのは
日本語音声のみです(他言語はスコープ外)。
...
- **日本語コンテンツのみ** — エンジンの音声モデルは日本語。非日本語テキストは非対応
...
全ワークスペース系ツールはオプショナルな `work_dir`(エージェントが
...
強制、ADR-0010)。またワークスペースディレクトリ自体が実ディレクトリで
なければなりません — `<work_dir>/<workspace_id>` が symlink だった場合、
リンク先に対して実行するのではなく、id の解決先を名指しして拒否します。
必須で既定値は無い — サーバーが選んだディレクトリは呼び出し側が
開けるとは限らないため。
...
ワークスペースから読むファイル(台本、キャスティング表、`master` がつなぐ各行の WAV)は
すべて先に判定します。`.env`、このサーバーの設定ディレクトリ内、資格情報ディレクトリ直下の
リンクの行き先のいずれかなら、読む前に `path_not_allowed` で拒否します(拒んだ WAV は
「無い」と数えます)。ファイルがあってもなくても同じ答えです。ワークスペースはそうした場所を
含み得て(`~/.ssh/config` がリンクする同期フォルダ内など)、台本として読むと中身が解析エラーに
出てしまうためです。別の Unicode 正規化の名前とハードリンクはまだ通ります。限界は
[ADR-0014](docs/ja/adr/0014-pathguard.ja.md) に挙げています。
