...
- Every file read from the workspace — script, casting table, line WAVs — is
  judged first: one that is a `.env`, lies in this server's config directory,
  or is where a link directly inside a credential directory points is refused
  with `path_not_allowed` before it is read, whether or not it exists (a
  refused line WAV counts as missing).
...
| path_not_allowed | use workspace-relative paths; symlinks out of the workspace are rejected, and so is a script or casting table that is a `.env`, lies in this server's config directory, or is where a link directly inside a credential directory points (whether or not it exists) |
