...
func newSynth(t *testing.T) (*synth.Synthesizer, *enginetest.Mock, *workspace.Workspace, *synth.CacheStore) {
	...
	ws, err := workspace.NewManager().EnsureUnder(t.TempDir(), "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCacheStoreRoundTrip(t *testing.T) {
	ws, err := workspace.NewManager().EnsureUnder(t.TempDir(), "cache-rt")
	if err != nil {
		t.Fatal(err)
	}
...
func TestCacheStoreCorruptIndexStartsFresh(t *testing.T) {
	ws, err := workspace.NewManager().EnsureUnder(t.TempDir(), "cache-corrupt")
	if err != nil {
		t.Fatal(err)
	}
