...
## Amendment (2026-09-22, v0.6.2): 連結リストの各項目を WAV に限る
...
- **実測**（ffmpeg 9.0.2、スクラッチの偽のファイル）: ワークスペースの行 WAV を、中身が `ffconcat version 1.0\nfile 'k'`
  ...
  音声が出力に入った。リストの入力全体に許可一覧（concat,wav）を付けても、中の項目が連結リストとして読めるので止まらない。
- **直した:** 連結リストを ffconcat 形式にし、各項目に `option format_whitelist wav` を付けた（細工した項目は開けずに
  失敗し、普通の WAV と無音の WAV の連結は通ることを確かめた）。リストの入力は `file` だけを読む。制御文字を含むパスは
  リストに書かない。
- 判定と読み取りの 2 段のあいだのすり替え（上の記述）は、受け入れた残余のまま（運用者の方針「完璧より総合的なリスク
  評価」）。pathguard は v0.3.0 に上げた（このサーバーでの挙動は変わらない）。
