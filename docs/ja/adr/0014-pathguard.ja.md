...
- **直した:** 無音の WAV・連結リスト・章・描画中の完成品は、ユーザーのキャッシュディレクトリの下（macOS では
  ...
  どちらのレーンでも書けない。途中で落ちたサーバーの残骸は、次のマスターが 1 日より古いものを片付ける。完成品だけを `os.Root` を通して一時名で作ってから名前を替えて置く（置かれたリンクはたどらずに置き換える）。連結リストは
...
- **受け入れた残余**（運用者の方針「完璧より総合的なリスク評価」）: 行 WAV を判定してから ffmpeg が開くまでの間に外への
  リンクへすり替えられると、外の、WAV として読めるファイルだけが読まれうる。キャッシュディレクトリも、サンドボックスの外で
  動く同じユーザーのプロセスには書ける —— それは利用者本人ができることの範囲である。pathguard は v0.3.0 に上げた
  （このサーバーでの挙動は変わらない）。
