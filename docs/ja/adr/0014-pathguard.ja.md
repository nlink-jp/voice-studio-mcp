...
- **直した:** 無音の WAV・連結リスト・章・描画中の完成品は、ユーザーのキャッシュディレクトリの下（macOS では
  `~/Library/Caches/voice-studio-mcp/master`）にマスターごとに作る専用ディレクトリで扱う。`$TMPDIR` にしないのは、
  gem-agent と lagent の書き込みレーンが `$TMPDIR` と `/private/tmp` を書けて、同じユーザーのプロセスは中を列挙できるから
  で、名前が推測できないことは防壁にならない（独立レビューが video-studio-mcp で実測、同じ作り）。キャッシュディレクトリは
  どのレーンでも書けない。途中で落ちたサーバーの残骸は、次のマスターが 1 日より古いものを片付ける。完成品だけを
  `os.Root` を通して一時名で作ってから名前を替えて置く（置かれたリンクはたどらずに置き換える）。連結リストは
  ffconcat 形式で各項目に `option format_whitelist wav` を付け、入力は `file` だけを読み、`-xerror` で拒まれた項目は途中で
  短い完成品にせず失敗にする。その裏返しとして、`ParseWAV` は通るが ffmpeg が読み途中でエラーを出す WAV（手で置いた
  壊れたファイルなど）も、欠けたままの完成品にならずマスターの失敗になる。制御文字を含むパスはリストに書かない。
- **受け入れた残余**（運用者の方針「完璧より総合的なリスク評価」）: 行 WAV を判定してから ffmpeg が開くまでの間に外への
  リンクへすり替えられると、外の、WAV として読めるファイルだけが読まれうる。キャッシュディレクトリも、どのサンドボックスにも
  入っていない同じユーザーのプロセス（利用者自身のシェルなど）には書ける —— それは利用者本人ができることの範囲である。pathguard は v0.3.0 に上げた
  （このサーバーでの挙動は変わらない）。
